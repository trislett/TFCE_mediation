...
* [tm_multimodality_multisurface_regression.py](https://github.com/trislett/TFCE_mediation/blob/master/tfce_mediation/tm_multisurface/tm_multimodality_multisurface_regression.py) is still in the early stages but it already has many new features including:
	...
	- Many surfaces can also be also analyzes at once with study-wide FWE correction. For example, analysis among all subcortical structures can be performed with TFCE, and corrected for all surfaces. Or even subcortical, cortical surfaces can be combined together in one analysis.
