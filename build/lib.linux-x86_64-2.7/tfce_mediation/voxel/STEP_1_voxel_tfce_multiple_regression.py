...
from tfce_mediation.stats import resid_covars, tval_int, calcF
from tfce_mediation.tfce import Surface
from tfce_mediation.func import write_voxelStat_img, create_adjac_voxel
...
def getArgumentParser(ap = ap.ArgumentParser(description = DESCRIPTION)):
	...
	ap.add_argument("-t", "--tfce", 
		help="TFCE setting: H, E, Connectivity (6 or 26 directions).", 
		nargs=3, 
		default=[2,1,26], 
		metavar=('H', 'E', '[6 or 26]'))
	return ap
...
def run(opts):
	...
	calcTFCE = Surface(float(opts.tfce[0]), float(opts.tfce[1]), adjac) # H=2, E=2, 26 neighbour connectivity
