...
if [ $# -eq 0 ]; then
	...
	echo "Usage: `basename $0` [-n <# of permutations>] [-p <# of processors> OR -c OR -f]"
...
for i in $(eval echo "{0..$forperm}"); do 
	echo ${SCRIPTPATH}/py_multiple_regression_randomise.py $(($i*100+1)) $(($i*100+100))
done > cmd_${medtype}_mult_regression_randomise_${current_time}

if [[ $p_opt = "gnu" ]]; then
	cat cmd_${medtype}_mult_regression_randomise_${current_time} | parallel -j ${num_processors}
fi

if [[ $p_opt = "condor" ]]; then
	${SCRIPTPATH}/tools/submit_condor_jobs_file cmd_${medtype}_mult_regression_randomise_${current_time}
fi
if [[ $p_opt = "fsl_sub" ]]; then
	fsl_sub -t cmd_${medtype}_mult_regression_randomise_${current_time}
fi
