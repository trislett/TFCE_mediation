...
ap = ap.ArgumentParser(description="Threshold 1D image.")
ap.add_argument("-i", "--input",  nargs=1, metavar=('*.mgh'), required=True)
ap.add_argument("-t", "--threshold",  nargs=1, required=True)
ap.add_argument("-n", "--neg", help='output negative threshold image', required=False, action="store_true")
opts = ap.parse_args()
