...
for i in $(eval echo "{0..$forperm}"); do 
	echo ${SCRIPTPATH}/surf_tfce_multiple_regression_randomise.py $(($i*100+1)) $(($i*100+100)) ${surf}
...
if [[ $p_opt = "condor" ]]; then
	submit_condor_jobs_file cmd_multipleregress_randomise_${surf}_${current_time}
fi
